...
while Current_cap > Endlife_cap
    ...
    if (Current_t) == 0
        ...
        Current_cap = Starting_cap-(1-func_BatteryDegrade(Current_t))
    end
    tvec = [tvec;Current_t];
...
%     ylim([0,30])
    ylabel('Battery MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Reserves_Init'),'fig');
    end
...
xlswrite('impacts.xlsx',label,1,'A37')
xlswrite('impacts.xlsx',impacts,1,'B37')
