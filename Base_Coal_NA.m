...
life=50400; %this will need to be changed with degradation
Endlife_cap = 0.5;
Starting_cap = 1;
...
%emissions factors
k_coal = 206*0.453592; % CO2 kg/mmbtu 
k_gas = 117*0.453592; % CO2 kg/mmbtu 
k_coal_nox = 0.227*0.453592;
k_gas_nox = 0.116*0.453592;
k_coal_so2 = 0.15713*0.453592;
k_gas_so2 = 0.001*0.453592;

k_co2=[k_coal,k_coal,k_gas,k_gas];
k_nox=[k_coal_nox,k_coal_nox,k_gas_nox,k_gas_nox]; 
k_so2=[k_coal_so2,k_coal_so2,k_gas_so2,k_gas_so2];

k_dollar=[k_dollar_coal,k_dollar_coal,k_dollar_gas,k_dollar_gas];
...
legend('load','wind')
fname = 'filepath';
saveas(gca, fullfile(fname,'load_New'),'fig');

%%
up_min_vec = [12,12,4,4];
down_min_vec = [12,12,4,4];
...
mpc.gen(1:4,9:10) = [340,136;300,120;80,32;80,32];
...
g1_ratio=g1_max/Hunter_max;
g2_ratio=g2_max/Hunter_max;

g1_cost=[coef_Hunter_cost(1)/g1_ratio,coef_Hunter_cost(2),coef_Hunter_cost(3)*g1_ratio];
...
%%
% Change reserve cost
Rcost = [9;10;4;4;9;10;4;4]*baseMVA;

c_on = [5.61*mpc.gen(1,9)+7.5*mpc.gen(1,9)*k_dollar_coal;5.61*mpc.gen(2,9)+7.5*mpc.gen(2,9)*k_dollar_coal;
    3.99*mpc.gen(3,9)+3.67*mpc.gen(3,9)*k_dollar_gas;3.99*mpc.gen(4,9)+3.67*mpc.gen(4,9)*k_dollar_gas]; % $ per occurance

c_off = [5.61*mpc.gen(1,9);5.61*mpc.gen(2,9);3.99*mpc.gen(3,9);3.99*mpc.gen(4,9)]; % $ per occurance
c_rES = 0.1;
...
while Current_cap > Endlife_cap
    ...
    for i = 1:n_interval
        ...
        p_i = p_noES((i-1)*n_generator+1:i*n_generator);%generation
        p1_noES(i,1) = p_i(1);
        p2_noES(i,1) = p_i(2);
        p3_noES(i,1) = p_i(3);
        p4_noES(i,1) = p_i(4);
        ...
        r_i = r_noES((i-1)*n_generator+1:i*n_generator);%reserves
        r1_noES(i,1) = r_i(1);
        r2_noES(i,1) = r_i(2);
        r3_noES(i,1) = r_i(3);
        r4_noES(i,1) = r_i(4);
    ...
    hline.Color='k';
    hline.LineStyle='--';
    legend('without energy storage','with energy storage','Location','northoutside')
    xlim([0,23])
    ...
    ylabel('Renewable MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Dispatch_Init_New'),'fig');
    ...
    ylabel('Battery MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Reserve_Init_New'),'fig');
    end
    ...
    for i = 1:length(t_vec)
        ...
        co2_total_noES(i,1) = co2_p; 
        so2_total_noES(i,1) = so2_p; 
        nox_total_noES(i,1) = nox_p; 
    ...
    nox_vec = [nox_vec;nox_delta];% positive if ES increased emissions
    ...
    xlim([0,23])
    ylabel('G4 MMBtu (NG)')
    saveas(gca, fullfile(fname,'Fuel_Init_New'),'fig');
    end
end
%%
    ...
    ylabel('Renewable MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Dispatch_Final_New'),'fig');
    ...
    ylabel('Battery MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Reserves_Final_New'),'fig');

total_co2 = sum(30*co2_vec)
total_so2 = sum(30*so2_vec)
total_nox = sum(30*nox_vec)
total_coal = sum(30*coal_vec)
total_gas = sum(30*gas_vec)
total_coal_MW=30*(sum(g1_vec)+sum(g2_vec))
total_gas_MW=30*(sum(g3_vec)+sum(g4_vec))

label={'Base_Coal_NA'};
impacts(1,:)=[total_co2,total_so2,total_nox,total_gas,total_coal,total_gas_MW,total_coal_MW];
xlswrite('impacts_Xu.xlsx',label,1,'A2')
xlswrite('impacts_Xu.xlsx',impacts,1,'B2')

%%
figure
plot(tvec,co2_vec,'-o','MarkerSize',4) % tvec is in hours not days
xlabel('Hours')
ylabel('CO_2 (kg)')
saveas(gca, fullfile(fname,'CO2_New'),'fig');

figure
plot(tvec,coal_vec,'-o',tvec,gas_vec,'-o','MarkerSize',4) % tvec is in hours not days
xlabel('Hours')
ylabel('Fuel (mmBtu)')
legend('Coal','Gas')
saveas(gca, fullfile(fname,'Fuel_New'),'fig');

figure
plot(tvec/24/30,g1,'-o',tvec/24/30,g2,'-o',tvec/24/30,g3,'-o',tvec/24/30,g4,'-o','MarkerSize',4)
legend('g1','g2','g3','g4')
xlabel('Months')
ylabel('Generation (MWh)')
saveas(gca, fullfile(fname,'Gen_ES_New'),'fig');

figure
plot(tvec/24/30,g1_nES,'-o',tvec/24/30,g2_nES,'-o',tvec/24/30,g3_nES,'-o',tvec/24/30,g4_nES,'-o','MarkerSize',4)
legend('g1_nES','g2_nES','g3_nES','g4_nES')
xlabel('Months')
ylabel('Generation (MWh)')
saveas(gca, fullfile(fname,'Gen_noES_New'),'fig');
