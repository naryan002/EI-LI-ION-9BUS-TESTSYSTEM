...
k_dollar_coal = 2.37; % dollar/mmbtu, 2014: 2.37, 2017: 2.22
k_dollar_gas = 5.00; % dollar/mmbtu, 2014: 5.00, 2017: 3.67
...
k_coal_so2 = 0.15713*0.453592; 
k_gas_so2 = 0.001*0.453592;

k_co2=[k_gas,k_coal,k_gas,k_gas];
k_nox=[k_gas_nox,k_coal_nox,k_gas_nox,k_gas_nox];
k_so2=[k_gas_so2,k_coal_so2,k_gas_so2,k_gas_so2];

k_dollar=[k_dollar_gas,k_dollar_coal,k_dollar_gas,k_dollar_gas];

Flag_wind=3.7; % 0 for no renewables 1 for renewables
Flag_solar=0;
t_vec = [0:23]';
...
%%
up_min_vec = [4,12,4.5,5];
down_min_vec = [4,12,4.5,5];
...
mpc.branch(:,6) = 1000*ones(size(mpc.branch,1),1);

% Change generator limits (ineq, linear)
mpc.gen(1:4,9:10) = [250,100;160,64;240,96;150,60]; 
...
%%
% Change reserve cost
Rcost = [5;10;4;4;5;10;4;4]*baseMVA; 

c_on = [3.99*mpc.gen(1,9)+3.67*mpc.gen(1,9)*k_dollar_gas;5.61*mpc.gen(2,9)+7.5*mpc.gen(2,9)*k_dollar_coal;
    3.99*mpc.gen(3,9)+3.67*mpc.gen(3,9)*k_dollar_gas;3.99*mpc.gen(4,9)+3.67*mpc.gen(4,9)*k_dollar_gas]; 

c_off = [3.99*mpc.gen(1,9);5.61*mpc.gen(2,9);3.99*mpc.gen(3,9);3.99*mpc.gen(4,9)];
c_rES = 0.1;
...
eff_loss=(eff-EOL_eff)/life*24*30;
...
for i = 1:n_interval
    ...
    mpc.gen(5,9:10) = [wind_forecast,0];
...
u_0 = ones(n_generator*u_0_length,1);
u_extend = [u_0;u];
...
while Current_cap > Endlife_cap
    ...
    Current_t = Current_t + Simu_interval 
    
    
    ESavailable = ESCap*Current_cap*ESallowed/2;
...
%     ylim([0,30])
    ylabel('Battery MW')
    xlabel('Hours')
    saveas(gca, fullfile(fname,'Reserve_Init'),'fig');
    end
    ...
    for i = 1:length(t_vec)
        ...
        co2_total_noES(i) = co2_p; %represents a day's worth of emissions
        so2_total_noES(i) = so2_p; %represents a day's worth of emissions
        nox_total_noES(i) = nox_p; %represents a day's worth of emissions
        Fuel_coal_noES(i) = coal_p;
        Fuel_gas_noES(i) = gas_p;
        Fuel_G1_noES(i) = Fuel1; %this is one day's worth of fuel
        Fuel_G2_noES(i) = Fuel2;
        Fuel_G3_noES(i) = Fuel3;
        Fuel_G4_noES(i) = Fuel4;
    end
    ...
    g1_ES=sum(gen1);
    g1_noES=sum(gen1_nES);
    g2_ES=sum(gen2);
    g2_noES=sum(gen2_nES);
    g3_ES=sum(gen3);
    g3_noES=sum(gen3_nES);
    g4_ES = sum(gen4);
    g4_noES=sum(gen4_nES);
    
    g1_daily_fuel=sum(Fuel_G1);
    g1_daily_fuel_noES=sum(Fuel_G1_noES);
    g2_daily_fuel=sum(Fuel_G2);
    g2_daily_fuel_noES=sum(Fuel_G2_noES);
    g3_daily_fuel=sum(Fuel_G3);
    g3_daily_fuel_noES=sum(Fuel_G3_noES);
    g4_daily_fuel=sum(Fuel_G4);
    g4_daily_fuel_noES=sum(Fuel_G4_noES);
    
    g1_fuel_vec = [g1_fuel_vec;g1_daily_fuel];
    g1_fuel_vec_noES = [g1_fuel_vec_noES;g1_daily_fuel_noES];
    g2_fuel_vec = [g2_fuel_vec;g2_daily_fuel];
    g2_fuel_vec_noES = [g2_fuel_vec_noES;g2_daily_fuel_noES];
    g3_fuel_vec = [g3_fuel_vec;g3_daily_fuel];
    g3_fuel_vec_noES = [g3_fuel_vec_noES;g3_daily_fuel_noES];
    g4_fuel_vec = [g4_fuel_vec;g4_daily_fuel];
    g4_fuel_vec_noES = [g4_fuel_vec_noES;g4_daily_fuel_noES];
    
   
    g1_vec = [g1_vec;g1_ES];
    g1_vec_noES = [g1_vec_noES;g1_noES];
    g2_vec = [g2_vec;g2_ES];
    g2_vec_noES = [g2_vec_noES;g2_noES];
    g3_vec = [g3_vec;g3_ES];
    g3_vec_noES = [g3_vec_noES;g3_noES];
    g4_vec = [g4_vec;g4_ES];
    g4_vec_noES = [g4_vec_noES;g4_noES];
...
total_coal_MW=30*(sum(g2_vec)-sum(g2_vec_noES))
total_gas_MW=30*((sum(g3_vec)+sum(g4_vec)+sum(g1_vec))-(sum(g3_vec_noES)+sum(g4_vec_noES)+sum(g1_vec_noES)))

label={'Curt_NG_W'};
impacts(1,:)=[total_co2,total_so2,total_nox,total_gas,total_coal,total_gas_MW,total_coal_MW];
xlswrite('impacts_Xu.xlsx',label,1,'A31')
xlswrite('impacts_Xu.xlsx',impacts,1,'B31')
...
figure
plot(tvec,coal_vec,'-o',tvec,gas_vec,'-o') % tvec is in hours not days this value is indended to be for one day
xlabel('Hours')
ylabel('Fuel (mmBtu)')
legend('Coal','Gas')
