...
k_co2=[k_coal,k_coal,k_gas,k_gas]; % for heavy coal
k_nox=[k_coal_nox,k_coal_nox,k_gas_nox,k_gas_nox]; % for heavy coal
k_so2=[k_coal_so2,k_coal_so2,k_gas_so2,k_gas_so2]; % for heavy coal

k_dollar=[k_dollar_coal,k_dollar_coal,k_dollar_gas,k_dollar_gas]; % for heavy coal

Flag_wind=1; % 0 for no renewables 1 for renewables
Flag_solar=0;
t_vec = [0:23]';
...
eff_loss=0; %13680 is the life of the battery
...
while Current_cap > Endlife_cap
    ...
    if (Current_t) == 0
        ...
        Current_cap = eff_BatteryDegrade_Xu(Current_t)
    end
    tvec = [tvec;Current_t];
    ...
    for i = 1:length(t_vec) %do something in this section 
...
xlswrite('impacts_Xu.xlsx',label,1,'A44')
xlswrite('impacts_Xu.xlsx',impacts,1,'B44')
